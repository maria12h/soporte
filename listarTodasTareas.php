...
<div class='ventana_flotante'>
	...
	<section id='secondary_bar'>
		<div class='user'>
			<p>".$_SESSION['UserA']." (<a>".$Nprocesos.":Mensajes </a>)</p>
		</div>
	...
	<section id='main' class='column'>
		...
		<article class='module width_full'>
			<header><h3>Listar todas las tareas</h3></header>
	<form action='buscartodostareas.php' method='POST'><input id ='buscar' name='nombrestoda' value='' type='text'> <input id ='btnbuscar' type='submit' value='Buscar tarea'></form >
			...
			";
			   ...
			   $peticion = "select* from tarea INNER JOIN tecnico on tarea.idtecnico=tecnico.idtecnico INNER JOIN rproceso on tarea.id_rproceso=rproceso.Num_reporte INNER JOIN area on area.idarea=rproceso.idarea INNER JOIN oficina on oficina.Idoficiana=rproceso.Idoficina WHERE tecnico.tipo='T' ORDER BY id_tarea,id_rproceso,fecha_asignacion_tarea ASC";
				...
				echo " <tr id='encabezado'>";
				     ...
				     echo "<td>Area</td>";
				     echo "<td>Personal solicitante</td>";
				...
				if($fila['Condicion']=="0")
					...
					{
echo"<tr id='estacien'>";
echo'<td>'.$fila['id_tarea']." ".'</td>'.'<td>'.$fila['fecha_asignacion_tarea'].'</td>'.'<td>'.$fila['Detalle']."".'<td>'.$fila['nombre']."".'</td>'.'<td>'.$fila['Estado'].":"."Completado".'</td>'.'<td>'.$fila['Nombreoficina'].'<td>'.$fila['nom_area'].'<td>'.$fila['nombre_personal']."".'</td>'."".'</td>'.'<td><a href="HabilitarTarea.php?idrepH='.$fila['id_rproceso'].'"><input type="submit"  id="modiboton" value ="Habilitar tarea" title="Modificar"> </a></td>';
					echo'</tr>';
				}
