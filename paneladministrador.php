...
<body style='Background-color:cian;'>
...
<div class='ventana_flotante'>
	...
	<header id='header'>
		...
		<hgroup>
			...
			<h2 class='section_title'></td>Lista de Problemas</h2>
	...
	<section id='secondary_bar' style='Background-color:cian;'>
		<div class='user'>
	...
	<aside id='sidebar' class='column' style='Background-color:#005;'>
		
	    <h3><img src='img/08.png'>Tareas</h3>
<div style='Background-color:#005;'>
		<ul class='toggle'>
		...
		<ul class='toggle'>
			...
			<li class='icn_add_user'><a href='InsertarIncidencia.php'>Añadir Incidencia</a></li>
	...
	<section id='main' class='column'>
		<article class='module width_full'>
			<header><h3>Estado de peticiones</h3></header>
		...
		<article class='module width_full'>
			...
			<header><center><h3>Lista de problemas </h3></center></header>
			<form action='buscarproblema.php' method='POST'><input id ='buscar' name='nombresb' value='' type='text'> <input id ='btnbuscar' type='submit' value='Buscar por Nombre' ></form >
		<table  border=1 width=950px;>
			";
                
				$peticion ="SELECT * FROM rproceso  INNER JOIN area on rproceso.idarea=area.idarea INNER JOIN oficina on oficina.Idoficiana=rproceso.Idoficina ORDER BY Num_reporte DESC";
				$resultado = mysqli_query($conexion, $peticion);
				...
				if($fila['Condicion'] =='1'){
					...
					//if($estado == 0){$diestado = "no servido";}else{$diestado = "servido";}
						echo'<td>'.$fila['Num_reporte']." ".'</td>'.'<td>'.$fila['nombre_personal'].'</td>'.'<td>'.$fila['Descripcion'].'</td>'.'<td>'.$fila['nom_area']."".'<td>'.$fila['Nombreoficina'].'<td>'.$fila['fecha']."".'</td>'.'<td><a href="Asignartareas.php?id='.$fila['Num_reporte'].'"><input id="botonen" type="submit" value="Asignar personal" title="Asignar personal"></a></td>';
						echo'</tr>';
						}else
						{
