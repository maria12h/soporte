...
<div class='ventana_flotante'>
	...
	<aside id='sidebar' class='column'>
		...
		<ul class='toggle'>
			<li class='icn_settings'><a href='paneladministrador.php'>Listar</a></li>
	...
	<section id='main' class='column'>
		...
		<table border=1 width=950px;>
			";
				$buscar=$_POST['nombrest'];
			   $peticion = "select* from tarea INNER JOIN tecnico on tarea.idtecnico=tecnico.idtecnico INNER JOIN rproceso on tarea.id_rproceso=rproceso.Num_reporte INNER JOIN area on area.idarea=rproceso.idarea  WHERE tecnico.tipo='T' and nombre like '%$buscar%'";
...
if($fila['Condicion'] =='1'){
					...
					{
echo"<tr id='estacien'>";
echo'<td>'.$fila['id_tarea']." ".'</td>'.'<td>'.$fila['fecha_asignacion_tarea'].'</td>'.'<td>'.$fila['id_rproceso']."".'<td>'.$fila['nombre']."".'</td>'.'<td>'.$fila['Estado'].":"."Completado".'</td>'.'<td>'.$fila['Detalle'].'<td>'.$fila['nom_area'].'<td>'.$fila['nombre_personal']."".'</td>'."".'</td>'.'<td><a href="DesabilitarTarea.php?idrep='.$fila['id_rproceso'].'"><input type="submit" value ="Terminar Tarea" title="Modificar"> </a>.'.'<td></td>';
					echo'</tr>';
				}
