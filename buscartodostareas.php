...
<div class='ventana_flotante'>
	...
	<header id='header'>
		<hgroup>
			...
			<h2 class='section_title'>Listar Tareas</h2><div class='btn_view_site'><a href='http://www.medialoot.com'>".$_SESSION['UserA']."</a><a id='cerrar' href='cerrars.php'>Cerrar Sesion</a></div>
		</hgroup>
	</header> 
...
<li class='icn_settings'><a href='listarproblema.php'>Listar problemas</a></li>
	...
	<section id='main' class='column'>
		...
		<article class='module width_full'>
			<center><header><h3>Lista de tareas encontradas</h3></header></center>
		<table border=1 width=950px;>
			";
				$buscar=$_POST['nombrestoda'];
			   $peticion = "select* from tarea INNER JOIN tecnico on tarea.idtecnico=tecnico.idtecnico INNER JOIN rproceso on tarea.id_rproceso=rproceso.Num_reporte INNER JOIN area on area.idarea=rproceso.idarea  WHERE tecnico.tipo='T'  and nombre_personal like '%$buscar%'";

				$resultado = mysqli_query($conexion, $peticion);
				echo " <tr id='encabezado'>";
				     echo "<td>Nº Tarea </td>";
				     echo "<td>Fecha  de asignacion</td>";
				     echo "<td>Nº de proceso</td>";
				...
				if($fila['Condicion']=="0")
					...
					{
echo"<tr id='estacien'>";
echo'<td>'.$fila['id_tarea']." ".'</td>'.'<td>'.$fila['fecha_asignacion_tarea'].'</td>'.'<td>'.$fila['id_rproceso']."".'<td>'.$fila['nombre']."".'</td>'.'<td>'.$fila['Estado'].":"."Completado".'</td>'.'<td>'.$fila['Detalle'].'<td>'.$fila['nom_area'].'<td>'.$fila['nombre_personal']."".'</td>'."".'</td>'.'<td><a href="DesabilitarTarea.php?idrep='.$fila['id_rproceso'].'"><input type="submit" value ="Terminar Tarea" title="Modificar"> </a>.'.'<td><a href="HabilitarTarea.php?idrepH='.$fila['id_rproceso'].'"><input type="submit" value ="habilitar Tarea" title="Modificar"> </a></td>';
					echo'</tr>';
				}
