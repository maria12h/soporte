...
$peticion = $peticion = "SELECT * FROM area  ORDER BY idarea ASC ";
...
<div class='ventana_flotante'>
	...
	<aside id='sidebar' class='column'>
		...
		<ul class='toggle'>
			<li class='icn_settings'><a href='listarproblema.php'>Listar  problema</a></li>
	...
	<section id='main' class='column'>
		...
		<article class='module width_full'>
			<header><h3>Lista de areas</h3></header>
<form action='buscararea.php' method='POST'><input id ='buscar' name='nombreso' value='' type='text'> <input id ='btnbuscar' type='submit' value='Buscar area'></form >
		...
		<table border=1 width=950px;>
			";
				...
				echo " <tr id='encabezado'>";
				     echo "<td>IdArea</td>";
				     echo "<td>Nombre Area</td>";
			
				echo " </tr>";
				while($fila = mysqli_fetch_array($resultado)) {
					echo"<tr id='colortecnico'>";
				//if($estado == 0){$diestado = "no servido";}else{$diestado = "servido";}
echo'<td>'.$fila['idarea']." ".'</td>'.'<td>'.$fila['nom_area'].'</td>'.'<td><a href="actualizarArea.php?idreT='.$fila['idarea'].'"><input type="submit" value="Modificar" id="modiboton"  title="Modificar"></a>'.'<td><a href="eliminarArea.php?idreTe='.$fila['idarea'].'"><input type="submit" id="booelimar" value="Elinimar" title="Eliminar"></a></td>';
					echo"</tr>";
				} 
