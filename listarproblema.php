...
$peticion = $peticion = "SELECT * FROM problema  ORDER BY idproblema ASC ";
...
<div class='ventana_flotante'>
	...
	<section id='secondary_bar'>
		<div class='user'>
			<p>".$_SESSION['UserA']."  (<a>".$Nprocesos.": Trabajadores </a>)</p>
		</div>
	...
	<aside id='sidebar' class='column'>
		...
		<ul class='toggle'>
			<li class='icn_settings'><a href='listarproblema.php'>Listar probelma</a></li>
	...
	<section id='main' class='column'>
		...
		<article class='module width_full'>
			<header><h3>Lista de problemas</h3></header>
<form action='buscarprob.php' method='POST'><input id ='buscar' name='nombreso' value='' type='text'> <input id ='btnbuscar' type='submit' value='Buscar Problema'></form >
		...
		<table border=1 width=950px;>
			";
				...
				echo " <tr id='encabezado'>";
				     echo "<td>IdProblema</td>";
				     echo "<td>Nombre Problema</td>";
			
				echo " </tr>";
				while($fila = mysqli_fetch_array($resultado)) {
					echo"<tr id='colortecnico'>";
				//if($estado == 0){$diestado = "no servido";}else{$diestado = "servido";}
echo'<td>'.$fila['idproblema']." ".'</td>'.'<td>'.$fila['nom_problema'].'</td>'.'<td><a href="actualizarproblema.php?idreT='.$fila['idproblema'].'"><input type="submit" value="Modificar" id="modiboton"  title="Modificar"></a>'.'<td><a href="eliminarproblema.php?idreTe='.$fila['idproblema'].'"><input type="submit" id="booelimar" value="Elinimar" title="Eliminar"></a></td>';
					echo"</tr>";
				} 
